...
class InfoOverlayer(object):
    ...
    def __init__(self, w=None, h=None, INFO_OVERLAY_SHOW=True):
        
        self.show = INFO_OVERLAY_SHOW
    ...
    def run(self, img_arr, fps, user_mode, user_throttle, user_angle, pilot_throttle, pilot_angle):
        ...
        self.add(f"user mode = {user_mode}")
        if user_mode == "user":
        ...
        if len(self.info_list) > 0:
            ...
            last_info_list = self.info_list.copy()
            self.info_list.clear()

        return new_img_arr, last_info_list
